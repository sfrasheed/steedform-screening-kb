...
- **`v6`** — the version. Any document or executable restating the rule must carry the tag inline as `[R-D5 v6]`. That is how drift is caught: a carrier holding `v5` when the Register says `v6` is stale by definition.
- **`LAW`** — the class.
