...
| **D5** | **Curves & shapes.** A curve polishes only up to **that MATERIAL's** limit; above it the curve is a **FACETED MITRE — makeable, never a decline**. ⚠ **The limit is MATERIAL-DEPENDENT, not a flat 40mm.** Above the limit → **🟡 + a MANDATORY documented CLIENT SIGN-OFF on the faceted appearance before fabrication.** It is **not blocking** and must never force Needs Review. Curves / radius ≤ 40mm and splayed or angled benches → **🟡 ALWAYS**. Faceting is elective from 40mm on any material and is **frequently the recommendation on veined stone** — a polished radius grinds through the vein, a facet preserves it. D5 is the single owner of splay / curve geometry; Me5 carries the template-capture echo | 🟡 + sign-off | `[R-D5 v6]`; `Edge Profile Capability Matrix` §2 |
...
`[R-D5 v6]` — thresholds, carried in full because they are reference data:
...
⛔ **The conservative fallback is MANDATORY and several carriers omit it** `[R-D5 v6]` — operative one-liner, cited:
...
- **ONE flag string, and it is fixed. Emit it verbatim:** `⚠ marble vs hard natural not derivable from the board; confirm before quoting the curve`. *(The former second string — "natural sub-type not recorded…" — is retired: `[R-D5 v6]` nominates a single canonical string.)*
