...
| D3 pencil on a face-printed surface | not distinguished from the 🟡 shaped-profile class | `[R-ANDON v2]`; `[R-PROFILE v6]`; ledger #157 | **⛔ CAPABILITY** | **RESET** the profile — steer to arris (or Sharknose/Euro) |
| D2 ogee | carried as a profile option | `[R-PROFILE v6]` (removed from all documents 23 Jul 2026); `[R-SCOPE v2]`; `[R-ANDON v2]` | **⛔ CAPABILITY** | **RESET** to a **lamb's tongue (Provincial)** |
| D1 30mm on anything but natural | *"any build ≥30mm … → 🟡, MATERIAL NAMED"* | `[R-30MM v1]`; `[R-PROFILE v6]`; `[R-ANDON v2]` | **⛔ CAPABILITY** (cannot be priced) | **RESET** the spec — do not price it `[R-30MM v1]` |
...
| **D11** | **Overhangs vs brand / thickness limits → 🟡 + MANDATORY FLAG-UPFRONT (the April rule):** a drawn overhang at or over the brand + thickness limit table → 🟡, support / steel / substrate confirmed and priced, and **raised at first builder contact, always**. Over-limit with no support shown is exactly the April failure mode. ✅ **The table is now carried — `05` §6.1b**, read off the manufacturers' own manuals 7 Sep 2026. Two rules ride on top of every figure in it: the unsupported overhang may never exceed **one third of the surface depth**, and the fabricator determines the support. ⚠ **Brands not yet in §6.1b have no threshold**, so an overhang on one of those is confirmed with the builder rather than scored against a number — a known gap, not a silent pass | 🟡 | Q5 ladder; `05` §6.1b |
...
`[R-D5 v9]` — thresholds, carried in full because they are reference data:
...
⛔ **The conservative fallback is MANDATORY and several carriers omit it** `[R-D5 v9]` — operative one-liner, cited:
...
- **ONE flag string, and it is fixed. Emit it verbatim:** `⚠ marble vs hard natural not derivable from the board; confirm before quoting the curve`. *(The former second string — "natural sub-type not recorded…" — is retired: `[R-D5 v9]` nominates a single canonical string.)*
