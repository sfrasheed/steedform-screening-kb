...
⛔ **UNRATED CUSTOMERS: ENGINEERED STONE ONLY** `[R-CLASS v2]`. Where the account is not
recorded, porcelain and natural stone are **declined — 🔴 blocking, Unqualified**. Read
the class from `Material Type` (`color_mm4qt817`) on the board, never from a brand list.

✅ **FOR A RATED CUSTOMER THIS DOCUMENT APPLIES IN FULL.** Porcelain and natural stone are
ordinary work — faceting thresholds, overhang tables, alfresco and outdoor rules are all
live. `[R-CLASS v2]` declines an unvetted customer on the least forgiving materials; it
does not narrow what SteedForm sells.
...
> ⚠ **Technical Standards §1 describes Smartstone Sintered as "full-bodied — vein colour matched into slab body, visible on 20mm edge profile" in its slab-specification table. That descriptor is not carried here.** It would permit a pencil profile on a sintered material, which the Edge Profile Capability Matrix §3a/§4 and `[R-PROFILE v6]` both exclude — porcelain and sintered remain excluded as a class regardless of what a slab table calls them. Where a Smartstone item's `Print Construction` reads blank, it fails closed to review like any other (§2).

---

## 2. Print construction — the pencil prohibition is by CONSTRUCTION, not class

**The test is construction, not class** `[R-PROFILE v6]` (corrected by the MD, 24 Aug 2026). Read **`Print Construction` (`color_mm6hz0dd`) live off Materials Library `5029570546`** `[R-REF v2]`:
...
**Worked example — QU-58883.** A **20mm Pencil** was priced on **Kaya Sienna**. The item reads `Engineered Stone` on the Materials Library and is face-printed per the Zenith/Kaya Manual May 2026 §7.2. **Both the quote and a full screen cleared it** — because both followed the rule exactly as it was then written: every carrier scoped the prohibition to "porcelain / sintered", which is a *proxy* for the real fact. `color_mm6hz0dd` Print Construction is now the authority for the fact, and blank fails closed `[R-PROFILE v6]`.

> ⚠ **Carrier note for the app.** Edge Profile Capability Matrix **§1a, §5.5, §5.6 and §6** still scope the pencil and lamination rules by CLASS ("porcelain / sintered") — those sections predate the 24 Aug 2026 correction that was applied to §3a. **Apply the construction-based test everywhere**, including wherever a class-scoped restatement appears. The class exclusion of porcelain and sintered still stands `[R-PROFILE v6]`; it is a floor, not the whole test.

---

## 3. Edge profile × material capability matrix

> **The `Edge Profile Capability Matrix` is the governing document for profile × material × machine capability.** Technical Standards §3 says so itself: *"The governing document is `Edge Profile Capability Matrix.md` … The rules below are the summary; **the matrix wins on any conflict.**"* `[R-PROFILE v6]` is the canonical rule; **do not price a combination this set rules out.**

### 3.1 The profile menu — capability by family

| Family | Sizes on the menu | Straight | Curved | Materials |
|---|---|---|---|---|
| **Arris** | 12 · 20\* · 30 · 40\* · 50 · 60 · 70 · 80 · 90 · 100 · 150 · 200 | ✓ EdiPlus (≤80 calibrated; >80 hand) | per §5 | **All materials** (build path per §4). **THE DEFAULT — and the default is ASSUMED where no profile is noted**, per the Terms & Conditions, changed only on the customer's request. An unstated profile is not missing information `[R-ASK v3]`. |
| **Pencil** | 12 · 20 · **30 (to add)** · 40 · 50 · 60 · 70 · 80 · 90 · 100 · 150 · 200 | ✓ flat polish + **HAND pencil** | per §5 | **🔴 NOT on any FACE-PRINTED surface** (§2) — read `Print Construction` live, blank fails closed `[R-PROFILE v6]`. Excludes all porcelain and sintered, **and the face-printed engineered ranges — Kaya, all ranges.** Full-body engineered, marble and hard natural only. Hand-finished at every size (#157) — "routine" ≠ "no labour". |
...
| **~~Ogee~~** | — | — | — | **NOT OFFERED — removed from all documents 23 Jul 2026 (MD).** Never quote it. **RESET the spec to a lamb's tongue (Provincial)** `[R-SCOPE v2]` `[R-PROFILE v6]` — which is 40mm CNC, straight and curved, engineered + marble only, so check the material before offering it. |
...
| Situation | Verdict |
|---|---|
| **Pencil specified on a face-printed surface** — the highest-frequency profile error to expect, since pencil is a common default request | **🔴 spec conflict — not offered. Steer to ARRIS** (or Sharknose/Euro) `[R-PROFILE v6]` |
...
| **Curved edge above the material's polish limit** | **🟡 specification + expectation step — never a decline.** The client is told it will be a faceted mitre and signs off the appearance. On veined stone, *recommend* faceted `[R-D5 v9]` |
...
| **Ogee specified** | **NOT OFFERED — RESET to a lamb's tongue (Provincial)** `[R-SCOPE v2]` `[R-PROFILE v6]`. A reset, not a decline: the enquiry continues on the supported profile and the client is told before the quote goes out (§6) |
...
- ✅ **A 40mm build on a face-printed material IS the mitred build, and asking for it is not an ambiguity** `[R-PROFILE v6]` (ruled 9 September 2026). Roughly nine such requests in ten arrive with straight edges. ⛔ Do not ask anyone to confirm it before quoting. **Only a CURVE, in porcelain or sintered, still needs a decision** — and that is the faceting sign-off in `[R-D5 v9]`, because a curved 40mm in those materials is faceted, never a polished radius.
- **Lamination is 20mm layers only.** There is no 30mm layer and no 20+40 combination. **A 50mm laminated build does not exist.**
- **A PRINTED FACE CANNOT BE LAMINATED** `[R-PROFILE v6]`. Ceramic, porcelain and sintered are never laminated, and neither is any material whose `Print Construction` (`color_mm6hz0dd`) reads `Face-printed`. **The print is the reason; the class is only a proxy** — the same correction MD made to the pencil rule on 24 Aug 2026, for the same reason.
- **Limestone and travertine are ruled not laminable — MD 23 Jul 2026** (§5). Unchanged and not a pending question.
- **The rest of natural stone — granite, quartzite, marble — is CASE BY CASE, resolved by a person per job** (ruled 7 Sep 2026, Stella Rasheed). It is not a property of the material recorded on a board, and no column will answer it: the answer is about the slab in front of you. ⚠ **This does not move any faceting threshold.** Lamination and faceting are different operations, and `[R-D5 v9]`'s table is unchanged — a stone that laminates still facets from 40 unless a rule says otherwise.
...
- **Scope: these rules run to 40mm.** 50–80mm has its own cost step; **>80mm is a hard cost cliff — flag it** `[R-PROFILE v6]`. 50mm and 60mm are *common*, not *free*: flag the thickness so it is priced, not as a risk. **A 50–80mm engineered mitred build scores 🟡 AMBER at D1, on the cost difference** (ruled 3 September 2026; the rung is stated in `03-risk-scoring.md` §3.3). **Engineered ≤ 40mm is 🟢.**
...
## 5. Curves and faceting `[R-D5 v9]`

**A curve is polished only up to that MATERIAL's limit; above it the curve is a FACETED MITRE — makeable, never a decline** `[R-D5 v9]`. The limit is **material-dependent, not a flat 40mm**, because only engineered and marble laminate.
...
| **Limestone · travertine** *(**RULED not laminable — MD 23 Jul 2026** `[R-D5 v9]`. This is settled, not pending: no carrier may reopen it, and it may be stated as MD law in a client-facing brief)* | single thickness | 40 | **40** |
| **Engineered · marble** *(laminable to 60)* | **40 (20+20 lamination)** | **40 — if the vein justifies it** | **60** ⚠ *neutral colours excepted — §5.2* |
...
**A curve above the limit scores 🟡 plus a MANDATORY documented client sign-off on the faceted appearance before fabrication. It is NOT blocking and must never force Needs Review** `[R-D5 v9]`.
...
### 5.2 A polished 60mm curve in engineered — neutral colours only `[R-D5 v9]`

**Verified by the MD, 7 September 2026.** The 60mm faceting threshold is **not absolute for
engineered**. With the correct glue, a 60mm laminated curve can be **polished rather than faceted**
on a **neutral colour**.

⛔ **A screen may not clear this. It is an escalation to Jordan, the OPS Manager, per job.**

- **Neutral colour.** A 60mm build is three layers and **two glue lines on the edge face**. On
  veined stone those joints break the vein twice — which is why §4.1 bars 60mm lamination on veined
  stone at all. A neutral colour has no vein to break. That is the entire reason this exception
  exists, and the reason it stops where it does.
- **The right glue.** Not any glue.
- **Jordan confirms, per job.** Not standing permission, and not something to infer from a colour
  looking plain in a photograph.

**So a 60mm curve on engineered in a neutral colour is 🟡 with the condition "confirm the 60mm
polished curve with Jordan (OPS Manager)"** — not "faceted, mandatory". On a **veined** colour at
60mm nothing changes: faceted, and §4.1's bar on the lamination still stands.

**How it is made:** the flat polish is done on the machine, and the **arris or pencil is applied by
hand** afterwards — the same hand finish `[R-PROFILE v6]` already requires of a pencil at every size.
...
**It is resolved by a person, per job. This is the standing position, not an interim one** (ruled 7 Sep 2026): nobody is waiting on a field, and the flag below is permanent. `[R-D5 v9]` requires:

> **Treat ANY natural stone as HARD NATURAL — faceted from 40mm — unless a human has confirmed it is marble.**

and a visible flag on the output. **There is now ONE flag string, and it is fixed** `[R-D5 v9]` (ruled 3 September 2026). Emit it **verbatim**:
...
| **The ogee profile** | **RESET** the spec to a **lamb's tongue (Provincial)** | Removed from all documents 23 Jul 2026 (MD) `[R-PROFILE v6]`. Provincial is 40mm CNC, **engineered + marble only** — check the material before offering it (§3.1) | — |
...
**Flush mounting is a mounting method, not an edge profile** — `[R-PROFILE v6]` governs edge profile × material and does not cover it; this is a separate capability refusal with its own tag `[R-FLUSH v1]`.
