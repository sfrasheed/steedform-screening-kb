...
| Amend `[R-PROFILE v3]` so blank means full body | One line, no data work — but it removes the fail-closed guard, and a new face-printed range added without marking would then be priced with a pencil. **That is QU-58883** |

**Until one is chosen:** flag it, but say why. The flag reads *"print construction not recorded"* — **never** *"may be face-printed"*. ⚠ **Do not amend the rule on the quiet.** `[R-PROFILE v3]` is a LAW and this is a Register change, not a board change.
