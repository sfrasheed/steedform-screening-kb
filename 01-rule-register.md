...
**Lamination = 20mm layers only** (→40mm, →60mm). **A PRINTED FACE CANNOT BE LAMINATED:** porcelain, ceramic and sintered never, and neither does any material whose `Print Construction` reads `Face-printed` — the print is the reason, the class is only a proxy. **Full-body engineered and marble laminate. Limestone and travertine are ruled not laminable (MD 23 Jul 2026).** The rest of natural stone — granite, quartzite — is **case by case, resolved by a person per job**; no column will carry it. ⚠ **Laminability does not set a faceting threshold** — they are different operations and `[R-D5 v9]`'s table is unchanged. Nothing below 40mm is mitred. Waterfall ends at 12/20/30/40mm are a standard chargeable install; a boxed island that cannot be glued in the factory is an **assemble-on-site** in every material.
✅ **A 40mm BUILD ON A FACE-PRINTED MATERIAL IS A MITRED BUILD, AND THAT IS THE ASSUMED INTENT. IT SCORES NOTHING.** Ruled by Stella Rasheed, 9 September 2026. Where a material cannot laminate, the mitred build off single thickness is **the only route there is**, and the customer asking for 40mm knows it — **about nine times in ten the request comes with straight edges and carries no ambiguity at all.** ⛔ **Do not raise it as something to resolve before quoting, do not route it to Needs Info, and do not ask a named person to confirm the only build that can be made.** The manufacturer agrees: the Zenith/Kaya Manual recommends a **40mm+ mitred edge** on face-printed to showcase the pattern (`05` §9).

⚠ **THE ONE CASE THAT STILL FLAGS: A CURVE, IN PORCELAIN OR SINTERED.** Both conditions, together. And there the question is **not lamination — it is FACETING**, which is `[R-D5 v9]`'s and already scored there: porcelain and sintered are **faceted from 40mm**, so a curved 40mm cannot be a polished radius and the customer is choosing a faceted one. **Straight edges in any material → nothing. A curve in engineered or marble → nothing** — they laminate and polish to 40mm.

**What this replaces.** A finding reading *"the 40mm build in a 12mm face-printed porcelain has to be resolved before quoting … a named person needs to confirm the mitred build before a price is put together"* was firing on ordinary work. A screen that asks a person to confirm the only possible build, on every 40mm porcelain job, spends the scarcest thing it has — a person's attention — where there is no decision to make.
...
**History:** **v6 **9 Sep 2026 (Stella Rasheed)**** — a 40mm mitred build on a face-printed material is the assumed intent and scores nothing; the flag narrows to a **curve in porcelain or sintered**, where it is `[R-D5 v9]`'s faceting question rather than a lamination one. The rule about what can laminate is **unchanged** — what changed is that being unable to laminate stopped being a reason to stop and ask. · **v5 **7 Sep 2026 (GM, recorded by Stella Rasheed)**** — Zenith Ottoman Grey and Rosè are **not** face-printed. v3 named them from the Zenith/Kaya Manual May 2026 §7.2; that is corrected, the two board rows were cleared the same day, and the rule now reads simply: **all Kaya is face-printed, all Zenith is full-body.** The by-name carve-out in `05` §9 is retired with it. · **v4 **7 Sep 2026 (Stella Rasheed)**** — lamination restated by construction rather than class, matching what MD did to the pencil rule in v3: a printed face cannot be laminated, whatever the material is called. Hard natural moves from "never" to **case by case per job**; limestone and travertine stay ruled not laminable per MD. The faceting thresholds are deliberately untouched — lamination and faceting are different operations, and the reasoning in `[R-D5 v9]` that ties the two together is flagged for review rather than acted on. · v1 23 Jul 2026 (MD profile audit) · v2 27 Jul 2026 (30mm rule split out to R-30MM) · **v3 24 Aug 2026 (MD: the pencil prohibition restated by CONSTRUCTION, not class.** Every carrier said "porcelain/sintered", which is a proxy for the real test — a printed face. QU-58883 priced a 20mm Pencil on **Kaya Sienna**, which reads `Engineered Stone` on the Materials Library and is face-printed per the Zenith/Kaya Manual May 2026 §7.2. Both the quote and a full screen cleared it, because both followed the rule as written. New Materials Library field `color_mm6hz0dd` **Print Construction** is now the authority for the fact; blank fails closed.)
...
### R-CLASS · v2 · LAW
**FOR AN UNRATED CUSTOMER ONLY: ENGINEERED STONE, OR THE ENQUIRY IS DECLINED — 🔴
BLOCKING, UNQUALIFIED.** Ruled by Stella Rasheed, 8 September 2026.

⛔ **SCOPE FIRST, BECAUSE IT IS THE WHOLE RULE.** This applies **only where the account is
not recorded** — no confident match `[R-ACCTMATCH v5]`, an unknown entity
`[R-ENTITY v1]`, or a blank Customer Tier, which is 131 of 191 accounts. **A RATED
CUSTOMER MAY SPECIFY ANY MATERIAL SteedForm sells.** Miller Kitchens ordering Vasari
Porcelain is ordinary work and must never be touched by this rule.

**Why it is scoped that way:** a rated customer has a history, terms and a relationship,
and porcelain or natural stone from them is a job SteedForm knows how to take. The same
material from a stranger is an unvetted customer on the least forgiving materials at
once, and that is the combination being declined — not the material.

It runs **before** drawings, scope or site *for an unrated enquiry*: what the material is
decides whether there is a job to screen at all.
...
**What this declines, FOR AN UNRATED ENQUIRY ONLY: 222 of 400 materials as at 8 Sep
2026** — Dekton (50), Vasari Porcelain (40), Sensa (35), Neolith (31), Caesarstone
Porcelain (24), Smartstone Sintered (24), Scalea (18). **For a rated customer it declines
nothing.**
...
⚠ **This is a ruled decline, not a capability stop** `[R-ANDON v2]`, and it is about the
CUSTOMER as much as the material. SteedForm fabricates porcelain and natural stone every
week for rated customers, and the knowledge for both is live, not historical —
`05` §5's faceting thresholds, `05` §6.1b's overhang tables, the alfresco and outdoor
rules. **All of it remains in force for rated customers**, which is most of the work.

⚠ **`[R-DEKTON v1]` still applies to everyone**, rated or not — the phase-out is a
material decision and is not scoped to unrated customers the way this rule is.
**Carried by:** `03 Risk Scoring` · `05 Material & Capability` · `09 Response Templates`

**History:** **v2 **8 Sep 2026 (Stella Rasheed)**** — scoped to unrated customers, which is
what was asked for and what v1 got wrong. v1 read as a global narrowing of the offering
...
- **Engineered · marble** — polished to a 40mm lamination (20+20); **faceted from 60mm**, ⚠ **except
  the neutral-colour 60mm case below**
...
✅ **A POLISHED 60mm CURVE IS POSSIBLE IN ENGINEERED — NEUTRAL COLOURS, RIGHT GLUE, AND JORDAN SAYS
SO.** Verified by the MD, 7 September 2026, recorded by Stella Rasheed. The 60mm faceting threshold
above is **not absolute for engineered**: with the correct glue, a 60mm laminated curve can be
polished rather than faceted **on a neutral colour**.

⛔ **THE SCREEN MAY NOT CLEAR THIS ON ITS OWN. It is an escalation to the OPS Manager (Jordan),
per job.** Three conditions, all of them, and the third is a person:

| | |
|---|---|
| **Neutral colour** | A 60mm build is three layers and **two glue lines on the edge face**. On a veined colour those joints break the vein twice, which is why `05` §4.1 bars 60mm lamination on veined stone outright. A neutral colour has no vein to break — that is the whole reason this exception exists and why it does not extend past it |
| **The right glue** | Not any glue. The capability depends on it |
| **Jordan confirms** | OPS Manager, per job. Not a standing permission, not something the screen infers from the colour looking plain |

**What a screen does with it:** a 60mm curve on engineered in a neutral colour is **🟡 with the
condition "confirm the 60mm polished curve with Jordan (OPS Manager)"** — *not* "faceted, mandatory".
On a **veined** colour at 60mm, nothing changes: faceted, and `05` §4.1 still bars the lamination.

**Fabrication, for context:** the flat polish is done on the machine and the **arris or pencil is
applied by hand** afterwards — consistent with `[R-PROFILE v6]`, where pencil is hand-finished at
every size.
...
occupancy `[R-OCCUPANCY v1]` · live Customer Tier `Do-Not-Quote`, or an unresolved account match `[R-C1 v4]` `[R-ACCTMATCH v5]` · out of area while the Supply Only conversion is undecided `[R-AREA v1]` · spec conflicts `[R-30MM v1]` `[R-PROFILE v6]` · below the one-slab minimum, non-A-tier `[R-C3 v1]` · the natural-stone rule set `[R-NATURAL v1]` · unknown-entity checks `[R-ENTITY v1]`.
