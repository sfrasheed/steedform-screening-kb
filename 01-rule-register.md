...
**Lamination = 20mm layers only** (→40mm, →60mm), **engineered + marble only**; porcelain/ceramic/sintered **never** laminated; nothing below 40mm is mitred. Waterfall ends at 12/20/30/40mm are a standard chargeable install; a boxed island that cannot be glued in the factory is an **assemble-on-site** in every material.
...
| Edge profile | generic (pencil, mitred 40) | exact profile, material-checked against the Capability Matrix **and against `Print Construction` `color_mm6hz0dd`** `[R-PROFILE v3]` |
...
occupancy `[R-OCCUPANCY v1]` · live Customer Tier `Do-Not-Quote`, or an unresolved account match `[R-C1 v4]` `[R-ACCTMATCH v5]` · out of area while the Supply Only conversion is undecided `[R-AREA v1]` · spec conflicts `[R-30MM v1]` `[R-PROFILE v3]` · below the one-slab minimum, non-A-tier `[R-C3 v1]` · the natural-stone rule set `[R-NATURAL v1]` · unknown-entity checks `[R-ENTITY v1]`.
